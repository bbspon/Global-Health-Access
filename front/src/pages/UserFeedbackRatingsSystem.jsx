...
import {
  ...
  Modal, ProgressBar, Image,
} from 'react-bootstrap';

// Mock APIs
const fetchCategories = () =>
  Promise.resolve(['Cleanliness','Wait time','Doctor care','Billing','Other']);
const fetchPartnerStats = () => Promise.resolve({
  overallRating: 4.2,
  ratingCount: 128,
...
const fetchReviews = () =>
  Promise.resolve([
    {
      id: 'r1', user: 'Verified User', time: '2025‑07‑10',
      emoji: '😊', comment: 'Good service', image: null,
      partnerReply: 'Thank you for your feedback.'
    },
    {
      id: 'r2', user: 'Verified User', time: '2025‑06‑20',
      emoji: '😠', comment: 'Overbilling in OPD', image: null,
      partnerReply: null
    },
  ]);
...
export default function UserFeedbackRatingsSystem({ partnerId }) {
  ...
  const [prompt, setPrompt] = useState({
    emoji: '', rating: 0, category: '', comment: '', image: null, anonymous: false
  });
  ...
  useEffect(() => {
    ...
    setAdminStats({
      total: 150, flagged: 12, avgRating: 4.2, lowPartners: ['Hospital A', 'Lab X']
    });
  }, []);
    ...
    <>
      <Navbar bg="light" expand="lg" className="mb-4">
        <Container>
          ...
          <Nav>
            <Nav.Link onClick={() => setStep('prompt')}>Leave Feedback</Nav.Link>
            <Nav.Link onClick={() => setStep('partnerProfile')}>View Partner Profile</Nav.Link>
            <Nav.Link onClick={() => setStep('adminDashboard')}>Admin Dashboard</Nav.Link>
          </Nav>
        </Container>
      </Navbar>
      <Container>
            ...
            <Card.Body>
              ...
              <Form>
                <Form.Group className="mb-3">
                  ...
                  <div>
                    {['😃','😐','😠'].map(e => (
                      <Button
                        key={e} variant={prompt.emoji===e?'primary':'outline-secondary'}
                        className="me-2" onClick={()=>setPrompt({...prompt, emoji:e})}
                      >{e}</Button>
                    ))}
                </div>
              </Form.Group>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={e=>setPrompt({...prompt, comment:e.target.value})}
                />
              </Form.Group>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="file"
                  onChange={e=>setPrompt({...prompt, image: e.target.files[0]})}
                />
              </Form.Group>
              <Form.Check
                type="checkbox"
                ...
                onChange={e=>setPrompt({...prompt, anonymous:e.target.checked})}
              />
              <Button variant="primary" onClick={handleSubmitPrompt}>Submit Feedback</Button>
            </Form>
          </Card.Body>
        </Card>
        )}
        {step === 'partnerProfile' && stats && (
          <Card>
            <Card.Body>
              ...
              <h5>Overall Rating: {stats.overallRating} / 5 ({stats.ratingCount} reviews)</h5>
              <ul>
              ...
              {reviews.map(r=>(
                <Card key={r.id} className="mb-2">
                  <Card.Body>
                    <div><strong>{r.emoji}</strong> – {r.user}, {r.time}</div>
                    <p>{r.comment}</p>
                    {r.image && <Image src={URL.createObjectURL(r.image)} fluid />}
                    {r.partnerReply && <Card.Footer><em>Partner replied:</em> {r.partnerReply}</Card.Footer>}
                    {!r.partnerReply && (
        ...
        {step === 'adminDashboard' && adminStats && (
          <Card>
            <Card.Body>
              ...
              <ProgressBar now={100*adminStats.flagged/adminStats.total}
                label={`${Math.round(100*adminStats.flagged/adminStats.total)}% flagged`} 
                className="mb-3" />
              <h6>Low-rated partners:</h6>
              <ul>
                {adminStats.lowPartners.map(p=><li key={p}>{p}</li>)}
              </ul>
